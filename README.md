...
- **Data script (local):** `pip install -r requirements.txt` then `python parkrun_data.py` or `python -m parkrun_etl` from the repo root (expects `_data/` layout as in git; implementation lives under `parkrun_etl/`)
